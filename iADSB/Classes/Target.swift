...
public extension IADSB {
    public class Target: IADSB.ServiceJson & Codable {
        ...
        public var location:CLLocation? {
            return IADSB.GPS.location(latitude:latitude, longitude:longitude, horizontalAccuracy:positionValid ? 10.0 : nil, verticalAccuracy:positionValid ? 10.0 : nil, altitude:altitude, courseTrue:bearingTrue, speed:speed , timestamp:timestamp)
        }
        ...
        public func distanceNM(fromLocation:CLLocation?) -> Double? {
            return IADSB.Constants.nm(meters: distance(fromLocation: fromLocation))
        }
        ...
        public func headingTrue(fromLocation:CLLocation?) -> Double? {
            return IADSB.Constants.headingTrue(from: fromLocation, to: self.location)
        }
        
        public func headingMagnetic(fromLocation:CLLocation?) -> Double? {
            return IADSB.Constants.headingMagnetic(from: fromLocation, to: self.location)
        }
